select *
from `united-pier-362412.covid1002.CovidDeaths` 
where continent is not null;

select *
from `united-pier-362412.covid1002.CovidVaccinations` ;
...
from `united-pier-362412.covid1002.CovidDeaths`tot
where continent is not null;
...
from `united-pier-362412.covid1002.CovidDeaths`dea
join `united-pier-362412.covid1002.CovidVaccinations`vac
...
--Create table
CREATE TABLE IF NOT EXISTS `united-pier-362412.covid1002.PopVac`
(`Continent` STRING,
  `Location`  STRING,
...
INSERT INTO `united-pier-362412.covid1002.PopVac`
(select dea.continent,dea.location,dea.date,vac.new_vaccinations,dea.population,sum(cast(vac.new_vaccinations as int)) OVER (Partition by dea.location,dea.date)as RollingPeopleVaccinated
from `united-pier-362412.covid1002.CovidDeaths`dea
join `united-pier-362412.covid1002.CovidVaccinations`vac
  on dea.date=vac.date
...
select continent,location,population,max(RollingPeopleVaccinated) as RollingPeopleVaccinated,(max(RollingPeopleVaccinated)/population*100) as Vaccination_rate
from `united-pier-362412.covid1002.PopVac`
where RollingPeopleVaccinated is not null
group by location,continent,population
order by 1,2;
